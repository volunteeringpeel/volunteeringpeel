...
create table if not exists event (
  ...
  add_info    boolean       not null default 0                  comment 'Is the additional information field required?',
...
create table if not exists user_shift (
  ...
  add_info          text                                          comment 'Additional information (transport, t-shirt size, etc.)',
  ...
  foreign key fk_user_shift_assigned_exec (assigned_exec) references user(user_id) on update cascade on delete set null,
...
create table if not exists sponsor (
  ...
  image       varchar(128)                                      comment 'Image link',
  website     varchar(256)                                      comment 'Website',
